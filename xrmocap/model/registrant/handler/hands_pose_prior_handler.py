...
_HandsPosePriorLoss = TypeVar('_HandsPosePriorLoss')


class HandsPosePriorHandler(BaseHandler):

    def __init__(self,
                 prior_loss: Union[_HandsPosePriorLoss, dict],
                 handler_key='hands_pose_prior',
                 device: Union[torch.device, str] = 'cuda',
                 logger: Union[None, str, logging.Logger] = None) -> None:
        """Construct a HandsPosePriorHandler instance compute smpl(x/xd) hand pose
        parameters, return a loss Tensor.

        Args:
            prior_loss (Union[BetasPriorLoss, dict]):
                An instance of HandsPosePriorLoss, or a config dict of
                HandsPosePriorLoss.
            handler_key (str, optional):
                Key of this input-handler pair.
                Defaults to 'hands_pose_prior'.
    ...
    def __call__(self,
        ...
        Args:
            left_hand_pose (torch.Tensor):
                The left hand pose parameters. In shape (batch_size, 45).
            right_hand_pose (torch.Tensor):
                The right hand pose parameters. In shape (batch_size, 45).
